...
export class GenderFormComponent implements OnInit{
  ...
  constructor(private _genderService: GenderService, private _route: ActivatedRoute, 
    ...
    if(this._validationService.validateGenderForm(this.initialGenderObj)){
      ...
      if(this.initialGenderObj.id === 0){
        ...
        this._genderService.addGenderData(this.initialGenderObj).subscribe({
          next: (res) => {
            // console.log(res);
            this._router.navigate(['/gender-list']);
        ...
        // Update Form
        this._genderService.updateGenderData(this.idParam, this.initialGenderObj).subscribe({
          next: (res) => {
            // console.log(res);
            this._router.navigate(['/gender-list']);
  ...
  handleUpdateForm(){
    if(this.initialGenderObj.id !== 0){
      this._genderService.getGenderDataById(this.initialGenderObj.id).subscribe({
        next: (data) => {
  ...
  markInvalidInputs(errorMessages: string[]) {
    this.isGenderNameValid = !errorMessages.some(error => error.includes("Gender Name"));
  }
}
